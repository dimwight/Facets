...
public abstract class InputField extends EvalField{
	...
	public void updateValue(String text){
		if(false)trace(".updateValue: text=",text);
		TypedNode source=codeds[0].source;
		source.setValues(new String[]{text});
	}
	@Override
	...
	final public Value[]inputs(){
		for(TreeCoded eval:codeds)
			if(eval instanceof CalculatedField)
				return((EvalCoded)eval).evaluate();
		return values();
	}
	final public int valueIndex(){
		Value values[]=values(),value=evaluate()[0];
