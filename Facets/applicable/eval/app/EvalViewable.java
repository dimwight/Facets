...
<p>{@link EvalViewable} maintains {@link SSelection}s as appropriate for its {@link SViewer}s
using {@link EvalForm#newInputsBuilder()} and {@link EvalForm#newPicksBuilder()}. 
 */
public final class EvalViewable extends StatefulViewable{
	HtmlFormBuilder inputs;
	public EvalViewable(final EvalForm framed){
	...
	protected void viewerSelectionEdited(SViewer viewer,Object edit,boolean interim){
		if(false)trace(".viewerSelectionEdited: ",((FormInput)edit).value);
	...
	protected SSelection newViewerSelection(SViewer viewer){
		...
		if(view instanceof InputView){
			if(inputs==null)inputs=form.newInputsBuilder();
